...
namespace SocialMedia.Api.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        #region Ejercicios de Dapper - Tarea
        [HttpGet("dapper/usuarios-sin-comentarios")]
        public async Task<IActionResult> GetUsuariosSinComentariosAsync()
        {
            var result = await _postService.GetUsuariosSinComentariosAsync();
            var response = new ApiResponse<IEnumerable<UsuariosSinComentariosResponse>>(result);
            return Ok(response);
        }

        [HttpGet("dapper/comentarios-3meses-adultos")]
        public async Task<IActionResult> GetCommentsLast3MonthsAdultsAsync()
        {
            var result = await _postService.GetCommentsLast3MonthsAdultsAsync();
            var response = new ApiResponse<IEnumerable<CommentsLast3MonthsAdultsResponse>>(result);
            return Ok(response);
        }

        [HttpGet("dapper/posts-sin-usuarios-activos")]
        public async Task<IActionResult> GetPostsSinUsuariosActivosAsync()
        {
            var result = await _postService.GetPostsSinUsuariosActivosAsync();
            var response = new ApiResponse<IEnumerable<PostsSinUsuariosActivosResponse>>(result);
            return Ok(response);
        }

        [HttpGet("dapper/usuarios-activos-con-comentarios")]
        public async Task<IActionResult> GetUsuariosActivosConComentariosAsync()
        {
            var result = await _postService.GetUsuariosActivosConComentariosAsync();
            var response = new ApiResponse<IEnumerable<UsuariosActivosConComentariosResponse>>(result);
            return Ok(response);
        }

        [HttpGet("dapper/posts-sin-comentarios")]
        public async Task<IActionResult> GetPostsSinComentariosAsync()
        {
            var result = await _postService.GetPostsSinComentariosAsync();
            var response = new ApiResponse<IEnumerable<PostsSinComentariosResponse>>(result);
            return Ok(response);
        }

        [HttpGet("dapper/posts-comentados-usuarios-activos")]
        public async Task<IActionResult> GetPostsComentadosUsuariosActivosAsync()
        {
            var result = await _postService.GetPostsComentadosUsuariosActivosAsync();
            var response = new ApiResponse<IEnumerable<PostComentariosUsersResponse>>(result);
            return Ok(response);
        }
        #endregion
    }
